...
const getUdmpToken = (files, options = {}) => {
  return new Promise((resolve, reject) => {
    ...
    files.forEach((fileItem, i) => {
      ...
      // file_name: 这个业务字段是要匹配旧有逻辑，是必传的，值为待上传的文件名
      pages.push({ bizdata: { file_name: fileItem.fileUuid } })
      bizdata.file_name = fileItem.fileUuid
    })
    const data = { pages, ...options, bizdata }
    fetchUdmpReserve({ data })
      .then(({ document, uptokens }) => {
        try {
          // 把返回数据拼到map中去
          document.pages.forEach((page, i) => {
            let item = fileInfosMap.get(page.bizdata.file_name)
            ...
            updateFileInfo(page.bizdata.file_name, item)
          })
          return resolve()
        } catch (error) {
...
const chunksFileUpload = (fileChunks, fileUuid) => {
  return new Promise(async (resolve, reject) => {
    ...
    for (let i = 0; i < fileChunks.length; i += MAX_CONCURRENCY_REQUEST) {
      let chunksFileInfos = fileChunks.slice(i, i + MAX_CONCURRENCY_REQUEST)
...
const fileDownload = (doc_id) => {
  return new Promise((resolve, reject) => {
    const data = { docId: doc_id, fileNameKey: 'file_name' }
