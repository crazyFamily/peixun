<template>
  <div class="establish__trainingCourseManagement">
    ...
    <el-card class="search-form-card">
      <el-form ref="form" :model="queryForm" inline>
        <el-form-item label="机构">
          <gc-institution v-model="queryForm.busiType" ref="institutionRef" style="width: 300px;"> </gc-institution>
        ...
        <el-form-item label="班主任">
          <gc-input
            ...
            v-model.trim="queryForm.realManager"
    ...
    <!-- 表格内容框 -->
    <el-card class="mt10">
      <table-pagination v-loading="getLoadingStatus('LOADING_ESTABLISH_CLASSMANAGEMENT_LIST')" :list="list" :tableList="tableList" :queryForm="queryForm" :getList="getList" ref="tablePaginationRef">
          <el-table-column label="操作" min-width="150" fixed="right">
              <template slot-scope="scope">
                  <div v-if="scope.row && scope.row.classStatus ==='02003'">
                <span class="active mr10" @click="jumpTrainCourseDetail(scope.row)">
                  查看
                </span>
                <span class="active mr10"  v-if="scope.row && scope.row.isFreeze ==='Y'" @click="freeze('N', scope.row)">
                  解冻
                </span>
                <span class="active mr10" v-if="scope.row && scope.row.isFreeze ==='N'" @click="jumpSeeEdit(scope.row)">
                  编辑
                </span>
                <span class="active mr10"  v-if="scope.row && scope.row.isFreeze ==='N'" @click="freeze('Y', scope.row)">
                  冻结
                </span>
              </div>
              <div v-if="scope.row && scope.row.classStatus ==='02002'">
                <span class="active mr10" @click="jumpTrainCourseDetail(scope.row)">
                  查看
                </span>
                <span class="active mr10" @click="jumpSeeEdit(scope.row)">
                  编辑
                </span>
                <span class="active mr10" @click="finishTrainNormalClass(scope.row)">
                  结训
                </span>
              </div>
              </template>
    ...
    <gc-model
      ...
      custom-class="establish__trainingCourseManagement__courseManagement"
      @close="courseManagementVisible = false"
      @cancel="courseManagementVisible = false"
      @comfirm="courseManagementComfirm"
    >
      <div class="title">
      ...
      <div class="menu-right">
        <div class="icons df">
          ...
          <!-- <span class="icons__upload">
                        <gc-fileInput width="auto" @change="batchUploadCourse">
                            <i class="icons__upload__icon"></i>
                            批量上传
                        </gc-fileInput>
                    </span> -->
          ...
          <span
            v-if="courseObj.isFreeze === 'N'"
            @click="openInvitation('invitation')"
            class="icons__lecturerInvitationLetter"
          >
            <i class="icons__lecturerInvitationLetter__icon"></i>
            讲师邀请函
          </span>
          <span
           v-if="courseObj.isFreeze === 'N'"
            @click="openInvitation('thanksLetter')"
            class="icons__lecturerThanksLetter"
          >
            <i class="icons__lecturerThanksLetter__icon"></i>
            讲师感谢函
          </span>
        </div>
      </div>
      ...
      >
        ...
        <el-table-column label="操作" width="106" class-name="icons no-line">
          <template v-slot="scope">
            <span @click="delTimetableItem(scope.$index)" class="active mr8">
            ...
            <span
              class="active mr8"
              @click="openInvitation('invitation', scope.row)"
              v-if="courseObj.isFreeze === 'N'"
            >
              <el-tooltip content="讲师邀请函" popper-class="icon-popper">
                <i
                  class="operation__lecturerInvitationLetter"
                  :class="{
                    'operation__lecturerInvitationLetter-disabled':
                      scope.row.isInvitationMail === 'Y' || scope.row.disabled
                  }"
                ></i>
              </el-tooltip>
            </span>
            <span
              class="active"
              @click="openInvitation('thanksLetter', scope.row)"
              v-if="courseObj.isFreeze === 'N'"
            >
              <el-tooltip content="讲师感谢函" popper-class="icon-popper">
                <i
                  :class="{
                    'operation__lecturerThanksLetter-disabled':
                      scope.row.isThanksMail === 'Y' || scope.row.disabled
                  }"
                  class="operation__lecturerThanksLetter"
                ></i>
              </el-tooltip>
            </span>
          </template>
    ...
    <gc-model
      title="调整学员"
      :visible="adjustStudentVisible"
      width="1200px"
      isAppend
      cancelText=""
      comfirmText="关闭"
      @close="adjustStudenClose"
      @cancel="adjustStudentVisible = false"
      @comfirm="adjustStudentComfirm"
    >
      <div class="between">
        <div>
          <span>请输入学员（UM号）</span>
        ...
        <!-- <div class="btns">
                    ...
                    <el-button class="button-60 button-blue">重置</el-button>
                </div> -->
      </div>
      <div class="menu-right mt20">
        <div class="icons no-line df">
          ...
          <span class="icons__upload">
            <gc-fileInput width="auto" @change="handleStudentUpload">
              <i class="icons__upload__icon"></i>
              批量上传
            </gc-fileInput>
          </span>
          <span class="icons__download" @click="studentTemplateDown">
            <i class="icons__download__icon"></i>
            模版下载
          </span>
    ...
    >
      <div v-if="timetableBatchAdd.title">
        <p>
          {{ timetableBatchAdd.title }}
        </p>
        <p
          v-for="error in timetableBatchAdd.errorList"
          :key="error.value"
          class="error-color"
        >
          {{ error.value }}
...
import { fetchClassManagerList } from '@/fetch/trainingCourseManagement'
import { ESTABLISH_COURSE_MAP, LINE_MAP, NO_STATUS,yesOrNoMap, TRAININGSTATE, TRAININGMODE } from '@/util/constants'
import {
  fetchQueryLadderCourse,
  fetchListClassProjectType,
  fetchQueryOutsideLadderCourse
} from '@/fetch/establish'
import { TRAINING_COURSE_TYPE } from '@/views/Establish/enum'
...
export default {
  ...
  methods: {
    getClassProjectType(){
      fetchListClassProjectType({ classType: TRAINING_COURSE_TYPE }).then(classDatas => {
        const { classTypeTwoList } = classDatas
        classTypeTwoList?.forEach(v => {
          this.list
            .find(v => v.labelKey === 'classTypeTwoList')
            .list.push({
              label: v.dataValue,
              value: v.dataEncode
            })
    ...
    getList() {
      const data = this.handleQueryForm()
      fetchClassManagerList({ data }).then(res => {
    ...
    openInvitation(type, row) {
      const isMail = {
        invitation: 'isInvitationMail',
        thanksLetter: 'isThanksMail'
      }
      if (row && (row[isMail[type]] === 'Y' || row.disabled)) {
        return
      }
      var selectList = row
        ? [row]
        : this.$refs.courseTableRef.$children[0].selection
    ...
    verifyTimetable(courseList) {
      ...
      const flag = courseList.some((v, i) => {
        ...
        if (!v.courseType) {
          this.$message.warning(`课程表第${i}行授课方式必选`)
          return true
        }
        if (!v.courseName) {
    ...
    // 学员调整模版下载
    studentTemplateDown() {
      templateDownload('/common/file/download/studentAdjust')
    },
    // 学员批量上传 事件
    handleStudentUpload($event) {
      ...
      this.$axios
        .post('/common/manager/batchClassStudent', data, dataHeaders)
        .then((res) => {
          const { code, data } = res.data
          if (parseInt(code) === 0) {
            this.handleSearchStudent()
            if (data.fail?.length) {
              manyLineHint(data.fail)
            }
          }
        })
    },
    ...
    // 跳转到培训班详情页
    jumpTrainCourseDetail(data){
      addAllowKeepAlive('/TrainingCourseDetail')
      this.$router.push({
        name: '培训班查看',
        params: {
          ...data,
    ...
    // 确认结训
    finishTrainNormalClass(item) {
      const { classId, classStatus, applyStatus } = item
      console.log(item)
      if (applyStatus === '02003') {
        return this.$message.warning('已结训培训班不可操作！')
      }
      // if (!["02002"].includes(classStatus)) {
      //     return this.$message.warning("进行中的培训班才能确认结训");
      // }
    ...
    // 冻结， 解冻
    freeze(isFreeze, rowList) {
      const item = rowList
      if (!item) {
        return this.$message.warning('请先选择培训班')
      }
      const { classId, isFreeze: freezeState, classStatus } = item
      if (!['02003'].includes(classStatus)) {
        return this.$message.warning('已结训的培训班才能冻结,解冻')
      }
      if (freezeState === isFreeze) {
        return this.$message.warning(
          `当前培训班已经是${isFreeze === 'Y' ? '冻结' : '未冻结'}`
        )
      }
      const data = { classId, isFreeze }
      this.$axios
        .post('/fn/classes/manager/modifyFreezeStatus', { data }, jsonHeaders)
        .then((res) => {
          const { code } = res.data
          if (code === 0) {
            this.$message.success('操作成功')
...
.search-form-card {
  ...
  :deep(.el-card__body .el-form .el-input) {
    min-width: 200px;
    width: inherit;
  }
