<template>
  <div>
    ...
    <ImportModel
      ...
      :importFetchFn="fetchImportBatchNewEmpTrainingClass"
      :faildExportFetchFn="fetchExportFailNewEmpTrainingClass"
      templateCode="NEW_EMP_TRAINING_CLASS"
...
import { 
  fetchListNewEmpTrainingClass, fetchExportNewEmpTrainingClass, 
  fetchCheckNewEmpTrainingClass, fetchSaveNewEmpTrainingClass,
  fetchDeleteNewEmpTrainingClass, 
  fetchImportBatchNewEmpTrainingClass,
  fetchImportNewEmpTrainingClass,
  fetchExportFailNewEmpTrainingClass,
} from '@/fetch/statementCentre'
import { useSubBranchManager } from '@/views/StatementCentre/hooks/useSubBranchManager'
...
const {
  ...
  baseQueryForm,
  apiDataList: fetchListNewEmpTrainingClass,
  apiExportDataList: fetchExportNewEmpTrainingClass
})
...
function deleteDataList () {
  if (selectedList.value?.length) {
    customHint('删除后将重新统计参训率和参训人员明细', '确认要删除此培训班？', '取消').then(async res => {
      ...
      const resDel = await fetchDeleteNewEmpTrainingClass({ classIds })
      queryDataList()
...
async function comfirmImportForm (data) {
  console.log(data)
  const params = {
    successList: data.successList.map(m => ({ classId: m.classId })),
    tip: 'Y'
  }
  await fetchImportNewEmpTrainingClass(params)
...
function classListRules (i) {
  return [
    ...
    { trigger: 'blur', validator: async (rule, value, cb) => {
      try {
        await fetchCheckNewEmpTrainingClass({ classId: value })
      } catch (err) {
  ...
  trainingClassRef.value.validate(async (valid) => {
    if (valid) {
      const postData = {
        ...
        classIds: baseInfoForm.classList.map(m => m.classId)
      }
      await fetchSaveNewEmpTrainingClass(postData)
