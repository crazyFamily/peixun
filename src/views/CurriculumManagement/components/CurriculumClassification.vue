<template>
  <div class="gc-curriculumClassification" ref="curriculumClassification">
    <div
      class="gc-curriculumClassification__input df"
      :class="{ disabled }"
      @click="openDropDown"
    >
      <span v-show="labelName" class="ellipsis-word f1">{{ labelName }}</span>
      <span v-show="!labelName" class="placeholder f1">
        {{ placeholder }}
      </span>
    ...
    blgStripLine: {
      ...
      handler(newValue, oldValue) {
        if (newValue && newValue !== oldValue) {
          getCurriculumTree(this.tagType, newValue, 1, this.url).then((data) => {
