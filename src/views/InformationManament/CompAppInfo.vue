...
export default {
  ...
  methods: {
    ...
    async getTypesLength() {
      let res = await fetchFinRealInfoCount({ type: 'app' })
    ...
    // 新增APP资讯
    addAppInfoHandle() {
      addAllowKeepAlive('/NewAppInfo')
      this.$router.push({ name: '新增/编辑APP资讯' })
    },
    // 编辑APP资讯
    editHandle(row) {
      addAllowKeepAlive('/NewAppInfo')
      this.$router.push({ name: '新增/编辑APP资讯', params: { id: row.id } })
    }
  }
}
</script>
