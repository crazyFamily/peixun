  ...
  <div class="module-info">
    ...
    <div class="course-item" v-for="(courseItem, index) in courseList" :key="index">
      <el-form-item
        label="学习形式"
        label-width="120px"
        :prop="`courseModuleList.${index}.fosterModality`"
        :rules="{ required: true, message: `学习形式必填` }"
      >
        <gc-select
          k="label"
          v="value"
      ...
      <el-form-item
        label="课程ID"
        label-width="120px"
        :prop="`courseModuleList.${index}.fosterProjectId`"
        :rules="{ required: courseItem.fosterModality !== PRACTICE_FILEUPLOAD, message: `课程ID必填` }"
      >
        <el-dropdown
          trigger="click"
        ...
        >
          <div class="gc-approval-chain">
            <el-input
              ...
              @focus="focusCourseInfo($event, courseItem, '学习形式')"
            >
            </el-input>
          </div>
      ...
      <el-form-item
        label="课程名称"
        label-width="120px"
        :prop="`courseModuleList.${index}.fosterProjectName`"
        :rules="{ required: courseItem.fosterModality === PRACTICE_FILEUPLOAD, message: `课程名称必填` }"
      >
        <el-input
...
import { LEARNING_FORMAT_OPTIONS, COURSE_ZN, PRACTICE_FILEUPLOAD } from '../../enum.js'
...
const selectStudyTypeHandle = (val, courseItem, index) => {
  ...
  nextTick(() => {
    emit('clearValidateModule', `courseModuleList.${index}.fosterProjectId`)
    emit('clearValidateModule', `courseModuleList.${index}.fosterProjectName`)
  })
}
...
const inputCourseInfo = (val, courseItem, index) => {
  if ((courseItem.fosterModality || courseItem.fosterModality === COURSE_ZN) && val) {
