  ...
  <div class="module-info">
    ...
    <div class="course-item" v-for="(courseItem, index) in courseList" :key="index">
      <el-form-item
        label="考试来源"
        label-width="120px"
        :prop="`qualificationModuleList.${index}.fosterModality`"
        :rules="{ required: true, message: `考试来源式必填` }"
      >
        <gc-select
          k="label"
          v="value"
      ...
      <el-form-item
        label="考试ID"
        label-width="120px"
        :prop="`qualificationModuleList.${index}.fosterProjectId`"
        :rules="{ required: courseItem.fosterModality !== PRACTICE_FILEUPLOAD, message: `考试ID必填` }"
      >
        <el-dropdown
          trigger="click"
        ...
        >
          <div class="gc-approval-chain">
            <el-input
              ...
              @focus="focusCourseInfo($event, courseItem, '考试来源')"
            >
            </el-input>
          </div>
      ...
      <el-form-item
        label="考试名称"
        label-width="120px"
        :prop="`qualificationModuleList.${index}.fosterProjectName`"
        :rules="{ required: courseItem.fosterModality === PRACTICE_FILEUPLOAD, message: `考试名称必填` }"
      >
        <el-input
...
import { QUALIFICATION_OPTIONS, PRACTICE_FILEUPLOAD } from '../../enum.js'
...
const selectStudyTypeHandle = (val, courseItem, index) => {
  ...
  emit('clearValidateModule', `qualificationModuleList.${index}.fosterProjectId`)
  emit('clearValidateModule', `qualificationModuleList.${index}.fosterProjectName`)
}
