<template>
  <gc-model :width="width" top="5vh" :title="title" :visible="isDialogShow" :showClose="false" @comfirm="handleComfirm" :isShowFooter="false">
    <slot></slot>
    <div class="hq-upload-box">
      <div v-if="!isPrview" class="upload-container">
        <el-upload
          drag
          action=""
          class="hq-upload"
      ...
      <table-pagination :list="tableColumns" :tableList="fileDataList" :queryForm="queryForm" :hide-pagination="true" ref="tablePaginationRef">
        <el-table-column v-if="!isPrview" slot="percentage" label="上传进度" width="150px">
        ...
        <el-table-column label="操作" min-width="110">
          <template slot-scope="scope">
            <span class="item active" @click="handleDownloadFile(scope.row)">下载</span>
            <span class="active ml10" v-if="!isPrview" @click="deleteFile(scope.$index)">删除</span>
          </template>
        </el-table-column>
      </table-pagination>
    </div>
    <p class="space" style="height: 60px"></p>
    <div v-if="isPrview" class="tanlents-upload-footer">
      <el-button class="item-button" @click="closeHandle" type="primary">关闭</el-button>
    </div>
    <div v-else class="tanlents-upload-footer">
...
import { uploadFile, getCurrentTime, downloadFileAsAlink } from '@/util/utils'
import { UPLOAD_STATUS_SCZ, UPLOAD_STATUS_SCCG, UPLOAD_STATUS_YSC } from '@/components/upload/emun'
import { uploadFileToUdmp, updateId as updateUdmpId, getUdmpFileInfo, fileDownload } from '@/util/udmp'
