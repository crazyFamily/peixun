    ...
    >
      ...
      <div class="el-image-viewer__canvas">
        <template v-for="(url, i) in urlList">
          <img v-show="i === index && typeof(url)=='string'"
            ref="img"
            :key="i"
          ...
          >
        <video v-show="i === index && typeof(url)=='object'" 
                  :key="i+20" controls autoplay
                  :src="currentImg?.url"></video>
        </template>
      </div>
    </div>
  </transition>
...
import { rafThrottle, isFirefox } from '@/util/utils';
...
export default {
  ...
  watch: {
    ...
    currentImg(val) {
      this.$nextTick(_ => {
        const $img = this.$refs.img[this.index];
        if ($img && !$img.complete) {
          this.loading = true;
        }
      });
